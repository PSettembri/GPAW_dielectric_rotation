...
class Chi0DysonEquations:
    ...
    def get_coulomb_scaled_kernel(self, xc='RPA', modified=False, **xckwargs):
    ...
    def calc_G(self,*args,**xckwargs):
        #chi0 = self.chi0
        qpd = self.chi0.qpd
        return self.coulomb.calculate_G(qpd)

    def calculate_vchi_symm(self, xc='RPA', direction='x', **kwargs):
        ...
        v_G, K_GG = self.get_coulomb_scaled_kernel(
            xc=xc, chi0_wGG=chi0_wGG, **kwargs)
...
class InverseDielectricFunction(DielectricFunctionData):
    ...
    def full_dielectric_matrix(self):

        """Get the full dielectric function ε_G(q,ω).
        ...
        along with the full dielectric function in the independent-
        particle random-phase approximation [Rev. Mod. Phys. 74, 601 (2002)],
        ...
        vchi0_WGG = self.vchi0_symm_wGG
        vchi_WGG = self.vchi_symm_wGG

        eps0_WG = np.zeros((len(self.wd.omega_w),len(self.v_G)),dtype=complex)
...
class DielectricFunctionCalculator:
    ...
    def get_calc_G(self,q_c=[0,0,0],direction='x',**xckwargs):
        """
            Gets the G vectors adopted for the considered q_c
        """
        return Chi0DysonEquations(self.chi0calc.calculate(q_c), self).calc_G()
...
class DielectricFunction(DielectricFunctionCalculator):
    ...
    def get_full_dielectric_function(self, *args, filename='df_full.csv', **kwargs):
        ...
        Returns
        -------
        df_NLFC_wG: np.ndarray
            Dielectric function without local field corrections for each G.
        df_LFC_wG: np.ndarray
            Dielectric functio with local field corrections for each G.
        """
...
def write_vector_response_function(filename, omega_w, rf0_wG, rf_wG):
    with open(filename, 'w') as fd:
        for ig in range(np.shape(rf0_wG)[1]):
            print(ig,file=fd)
            print(' ',file=fd)
